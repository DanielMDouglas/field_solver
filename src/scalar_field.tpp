...
template <typename T>
scalarField<T>::scalarField(std::vector <double> x,
			    std::vector <double> y,
			    std::vector <double> z,
			    T init_val)
{
			    ...
			    std::function <T (double, double, double)> f)
{
  ...
  for ( int i = 0; i < xSize; i++ ) {
	...
	// std::cout << x[i] << '\t' << y[j] << '\t' << z[k] << '\t' << f(x[i], y[j], z[k]) << std::endl;
...
template <typename T>
scalarField<T>::scalarField(boundary bound,
			    int Nx,
			    int Ny,
			    int Nz,
			    std::string type)
...
template <typename T>
scalarField<T>::scalarField(std::string filename)
{
...
template <typename T>
void scalarField<T>::set(int i, int j, int k, T value)
{
  values[ySize*zSize*i + zSize*j + k] = value;
}

template <typename T>
T scalarField<T>::get(int i, int j, int k)
{
  return values[ySize*zSize*i + zSize*j + k];
}

template <typename T>
void scalarField<T>::print_to_file(std::string filename)
{
...
template <typename T>
T scalarField<T>::interpolate(std::vector <double> pos)
{
...
template <typename T>
std::vector <T> scalarField<T>::interpolate_grad(std::vector <double> pos)
{
  std::vector <double> spacing = {0, 0, 0};
